...
export const SYNC_SOURCES: SyncSource[] = [
  ...
  { key: 'incidents', sheetId: '7165378768621444', tab: 'raw_incidents', campusCol: 'Campus Code', monthCol: 'Reporting Month', valueCol: 'Total Incident', hasMonth: true },

  // Pie chart â Incidents by Campus (no Incident Type column in new workspace)
  { key: 'v2_incident_types', sheetId: '7165378768621444', tab: 'raw_v2_incident_types', campusCol: 'Incident Type', valueCol: 'Total Incident', monthCol: 'Reporting Month', hasMonth: true, isolateFromCampusSet: true, realCampusCol: 'Campus Code' },

  // V2 KPIs â matched to GAS SyncService.gs
  { key: 'v2_hs_committee', sheetId: '435993944477572', tab: 'raw_v2_hs_committee', campusCol: 'Committee', plannedCol: 'Meeting Planned', actualCol: 'Meeting Conducted', monthCol: 'Reporting Month', hasMonth: true, isolateFromCampusSet: true, yesNoCount: true },
  { key: 'v2_findings_on_time', sheetId: '4947401822392196', tab: 'raw_v2_findings_on_time', campusCol: 'Campus Code', plannedCol: 'No. of Findings Closed', actualCol: 'No. of Findings Due', monthCol: 'Primary', hasMonth: true },
  { key: 'v2_risk_closed', sheetId: '7323092115214212', tab: 'raw_v2_risk_closed', campusCol: 'Campus Code', plannedCol: 'Total Risk Assessments Registered', actualCol: 'Risk Assessment Closed', monthCol: 'Primary', hasMonth: true },
  { key: 'v2_risk_validated', sheetId: '7323092115214212', tab: 'raw_v2_risk_validated', campusCol: 'Campus Code', plannedCol: 'Total Risk Assessments Registered', actualCol: 'Risk Assessment and Validation', monthCol: 'Primary', hasMonth: true },
  ...
  // { key: 'v2_hs_kpi_report', reportId: '4811266391494532', tab: 'raw_v2_hs_kpi_report', campusCol: 'Campuses', valueCol: 'Submitted', monthCol: 'Primary', hasMonth: true }, // Disabled: report 404
  ...
  { key: 'v2_safe_working', sheetId: '1693592581001092', tab: 'raw_v2_safe_working', campusCol: 'Campus Code', plannedCol: 'No. of SOPs Verified', actualCol: 'No. of SOPs Implemented', monthCol: 'Primary', hasMonth: true },
  ...
  { key: 'v2_hazard_id', sheetId: '7323092115214212', tab: 'raw_v2_hazard_id', campusCol: 'Campus Code', plannedCol: 'Total Controls Sampled', actualCol: 'Implemented Controls', monthCol: 'Reporting Month', hasMonth: true },
  ...
  { key: 'v2_planned_training', sheetId: '8549734774951812', tab: 'raw_v2_planned_training', campusCol: 'Campus Code', plannedCol: 'Planned (Yes/No)', actualCol: 'Planned (Yes/No)', monthCol: 'Reporting Month', hasMonth: true, yesNoCount: true },
  ...
  { key: 'v2_mgmt_review_actions', sheetId: '3267265049874308', tab: 'raw_v2_mgmt_review_actions', campusCol: 'Campus Group', monthCol: 'Reporting Month', hasMonth: true, isolateFromCampusSet: true },
];
...
export async function fetchSheet(sheetId: string, token: string): Promise<Record<string, any>[]> {
  ...
  // Exclude parent/summary rows to avoid double-counting
  const parentIds = new Set<number>();
  data.rows.forEach((row: any) => { if (row.parentId) parentIds.add(row.parentId); });
  return data.rows
    .filter((row: any) => !parentIds.has(row.id))
    .map((row: any) => {
...
export function processSource(src: SyncSource, records: Record<string, any>[]): KpiRow[] {
  const filtered = records.filter(r => { const c = String(r[src.campusCol] || '').trim(); const cc = String(r['Campus Code'] || '').trim(); return c !== 'ADC' && cc !== 'ADC'; });
  return filtered.map(r => {
    ...
    if (src.yesNoCount) {
      ...
      planned = (pVal === 'yes' || pVal === 'true' || pVal === '1') ? 1 : 0;
      actual = (aVal === 'yes' || aVal === 'true' || aVal === '1') ? 1 : 0;
    } else {
    ...
    const realCampus = src.realCampusCol ? String(r[src.realCampusCol] || '').trim() || undefined : undefined;
    return { campus, month, planned, actual, value, realCampus } as KpiRow;
